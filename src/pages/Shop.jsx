...
function ProductCard({ product, index }) {
  ...
  const hoverImage = product.images && product.images.length > 2
      ...
      <div className="relative overflow-hidden bg-surface" style={{ aspectRatio: '3 / 4' }}>
        ...
        {product.inStock === false && (
          <div className="absolute inset-0 bg-void/60 flex items-center justify-center">
            <span className="caption" style={{ color: 'var(--color-bone)', letterSpacing: '0.12em' }}>
              Sold out
            </span>
          </div>
        )}
...
export default function Shop() {
  ...
  return (
    <>
      <Helmet>
        <title>Shop the Collection — Tee's and Steeze | Buy Streetwear Online Nigeria</title>
        <meta name="description" content="Shop unisex streetwear from Tee's and Steeze. Graphic tees, hoodies, jerseys, pocket shirts, and armless pieces. Nationwide delivery. Limited drops." />
        ...
        <meta property="og:image" content="https://teesandsteeze.com/og-shop.jpg" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Shop — Tee's and Steeze" />
        <meta name="twitter:description" content="Not just clothes. The steeze. Shop the drop." />
      </Helmet>

      <main>
        {/* ── HEADER ── */}
        <section aria-label="Shop header" className="container" style={{ paddingTop: 'clamp(7rem, 12vw, 10rem)', paddingBottom: '0' }}>
          <h1 className="display-lg animate-fade-up">The Collection.</h1>
        </section>

        {/* ── FILTER BAR ── */}
        <section aria-label="Category filters" className="container" style={{ paddingTop: '2rem', paddingBottom: '3rem' }}>
          <nav aria-label="Product categories" className="flex flex-wrap gap-2 animate-fade-up animate-delay-100">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: '1px solid',
                    borderColor: isActive ? 'var(--color-bone)' : 'var(--color-border)',
                    background: isActive ? 'var(--color-bone)' : 'transparent',
                    color: isActive ? 'var(--color-void)' : 'var(--color-stone)',
                    borderRadius: '0',
        ...
        {/* ── PRODUCT GRID ── */}
        <section id="product-grid" aria-label="Products" className="container" style={{ paddingBottom: 'clamp(4rem, 8vw, 6rem)' }}>
          {loading ? (
            <ShopLoading />
          ) : filtered.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
            ...
            <div className="text-center" style={{ paddingTop: '4rem', paddingBottom: '4rem' }}>
              ...
              <button onClick={() => handleFilter('all')} className="link-cta mt-4">
                See all pieces →
              </button>
            </div>
          )}
        ...
        {/* ── DROP SIGNUP STRIP ── */}
        <section aria-label="Drop signup" style={{ borderTop: '1px solid var(--color-border)', borderBottom: '1px solid var(--color-border)' }}>
          <div className="container flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6" style={{ paddingTop: '2.5rem', paddingBottom: '2.5rem' }}>
            <div className="flex-shrink-0">
              <p className="font-body font-medium text-bone" style={{ fontSize: '0.9375rem' }}>Next drop coming.</p>
              <p className="font-body text-stone mt-0.5" style={{ fontSize: '0.8125rem' }}>Early access to every release. No spam. Just the drop.</p>
            </div>
            {!signupDone ? (
              <form onSubmit={handleSignup} className="flex gap-3 w-full sm:w-auto">
                <input type="text" value={signupValue} onChange={(e) => setSignupValue(e.target.value)} placeholder="WhatsApp or email" className="input flex-1 sm:flex-none" style={{ minWidth: '220px' }} required aria-label="WhatsApp number or email for drop notifications" />
                <button type="submit" className="btn-signup whitespace-nowrap">Get early access</button>
              </form>
            ) : (
              <p className="font-body font-medium" style={{ fontSize: '0.875rem', color: 'var(--color-steeze-green)' }}>You're on the list. ✓</p>
            )}
          </div>
        </section>
      </main>
